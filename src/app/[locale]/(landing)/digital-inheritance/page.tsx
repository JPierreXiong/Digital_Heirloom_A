...
export async function generateMetadata({
  ...
  const t = await getTranslations('pages.digitalInheritance');

  const canonicalUrl =
    locale !== envConfigs.locale
      ? `${envConfigs.app_url}/${locale}/digital-inheritance`
      : `${envConfigs.app_url}/digital-inheritance`;
...
export default async function DigitalInheritancePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const page: Landing = {
    ...
    // Focus on technical architecture which explains Fragment A/B
