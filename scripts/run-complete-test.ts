/**
 * 完整�?Edge Function 资产释放逻辑测试脚本
 * 自动完成：创建测试数�?-> 调用 Edge Function -> 验证结果 -> 清理数据
 * 
 * 执行方式�? * npx tsx scripts/run-complete-test.ts
 */
...
if (!SUPABASE_SERVICE_ROLE_KEY) {
  console.error('�?错误: SUPABASE_SERVICE_ROLE_KEY 未设�?);
  console.error('   请设置环境变量或�?.env.local 中配�?);
  process.exit(1);
}
...
async function createTestData(): Promise<TestData> {
  ...
  if (vaultError) {
    throw new Error(`创建保险箱失�? ${vaultError.message}`);
  }
  console.log(`�?保险箱创建成�?(ID: ${vaultId})`);

  // 3. 创建预警事件�? 天前，超过宽限期�?  const warningEventId = crypto.randomUUID();
  ...
  // 4. 创建受益人（包含完整地址信息�?  const beneficiaryId = crypto.randomUUID();
  ...
  if (beneficiaryError) {
    throw new Error(`创建受益人失�? ${beneficiaryError.message}`);
  }
...
async function verifyResults(testData: TestData): Promise<boolean> {
  ...
  // 1. 检查保险箱状态（期望: released�?  const { data: vault, error: vaultError } = await supabase
    .from('digital_vaults')
    .select('id, status')
  ...
  if (vaultError) {
    console.error(`�?查询保险箱失�? ${vaultError.message}`);
    allPassed = false;
  } else {
    if (vault.status === 'released') {
      console.log(`�?保险箱状�? released (正确)`);
    } else {
      console.log(`�?保险箱状�? ${vault.status} (期望: released)`);
      allPassed = false;
    }
  }

  // 2. 检查受益人状态（期望: notified�?  const { data: beneficiary, error: beneficiaryError } = await supabase
    .from('beneficiaries')
  ...
  if (beneficiaryError) {
    console.error(`�?查询受益人失�? ${beneficiaryError.message}`);
    allPassed = false;
  } else {
    if (beneficiary.status === 'notified') {
      console.log(`�?受益人状�? notified (正确)`);
    } else {
      console.log(`�?受益人状�? ${beneficiary.status} (期望: notified)`);
      allPassed = false;
    }
  ...
  // 3. 检查资产释放事�?  const { data: events, error: eventsError } = await supabase
    .from('dead_man_switch_events')
  ...
  if (shippingError) {
    // 物流记录表可能不存在或未配置，不算错�?    console.log(`⚠️  物流记录: 查询失败（可能未配置 ShipAny API）`);
  } else {
    if (shippingLogs && shippingLogs.length > 0) {
      console.log(`�?物流记录: 已创�?(追踪�? ${shippingLogs[0].tracking_number || 'N/A'})`);
    } else {
      console.log(`⚠️  物流记录: 未创建（可能未配�?ShipAny API）`);
    }
  }
...
async function runCompleteTest() {
  ...
  try {
    ...
    if (passed) {
      ...
      console.log('   - 保险箱状态已更新�?released');
      console.log('   - 受益人状态已更新�?notified');
      console.log('   - 释放令牌已生�?);
      console.log('   - 资产释放事件已记�?);
    } else {
      console.log('⚠️  部分测试失败，请检查：');
      console.log('   1. Edge Function 是否已部�?);
