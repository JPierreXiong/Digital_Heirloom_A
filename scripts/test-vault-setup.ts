/**
 * 测试脚本：模拟完整的保险箱设置流�? * 
 * 功能�? * 1. 模拟用户登录（需要有效的 session�? * 2. 完成 Step 1-4 设置流程
 * 3. 验证保险箱创建成�? * 4. 验证受益人创建成�? * 5. 验证流程闭环
 */
...
class VaultSetupTester {
  ...
  async run() {
    ...
    try {
      // Step 1: 设置主密码并生成恢复�?      await this.step1_MasterPassword();

      // Step 2: 添加数字资产
      await this.step2_AddAssets();

      // Step 3: 添加受益�?      await this.step3_AddBeneficiaries();

      // Step 4: 设置触发规则并完�?      await this.step4_CompleteSetup();

      // 验证最终结�?      await this.verifyVaultCreated();
  ...
  private async step1_MasterPassword() {
    console.log('📝 Step 1: 设置主密�?..');
    
    try {
      // 生成测试用的 vaultId
      const testVaultId = getUuid();
      
      // 生成恢复�?      const recoveryKit = await generateRecoveryKit(TEST_CONFIG.masterPassword, testVaultId);
  ...
  private async step2_AddAssets() {
    ...
    try {
      ...
      // 客户端加�?      const { encryptedData, salt, iv } = await encryptData(
  ...
  private async step3_AddBeneficiaries() {
    console.log('📝 Step 3: 添加受益�?..');
    
    try {
      const setupData = (global as any).testSetupData;
      if (!setupData) {
        throw new Error('Setup 数据未找�?);
      }

      // 验证受益人数�?      for (const beneficiary of TEST_CONFIG.beneficiaries) {
  ...
  private async step4_CompleteSetup() {
    ...
    try {
      ...
      // 调用 API（需要有效的 session�?      const response = await fetch(`${TEST_CONFIG.baseUrl}/api/digital-heirloom/vault/initialize`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || result.code !== 200) {
        throw new Error(result.message || '初始化失�?);
      }

      // 验证返回的数�?      if (!result.data || !result.data.vault) {
        throw new Error('无效的响应数�?);
      }
