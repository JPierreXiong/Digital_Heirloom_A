#!/usr/bin/env tsx
/**
 * 生成 AUTH_SECRET 并更�?.env.local 文件
 */
...
console.log(`生成�?AUTH_SECRET: ${authSecret}\n`);
...
if (oldPattern.test(content)) {
  ...
  console.log('�?已更�?.env.local 中的 AUTH_SECRET');
} else {
  // 如果没有找到，添加到文件末尾
  content += `\nAUTH_SECRET=${authSecret}\n`;
  console.log('�?已添�?AUTH_SECRET �?.env.local');
}
...
console.log('   2. 运行环境变量检�? npx tsx scripts/check-env.ts\n');
