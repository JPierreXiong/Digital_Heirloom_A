 ...
 * 
 * 环境变量:
 *   ADMIN_EMAIL=xiongjp_fr@163.com (可选，默认使用此邮�?
 */
...
async function setAdminUser() {
  try {
    ...
    if (!adminUser) {
      ...
      console.log('\n💡 提示:');
      console.log('   1. 请确保该邮箱已注册并登录过系�?);
      console.log('   2. 如果未注册，请先访问登录页面完成注册');
      console.log('   3. 或者使用环境变量指定其他邮�?');
    ...
    console.log('   1. 退出并重新登录以刷新权�?);
    console.log('   2. 访问 http://localhost:3000/admin/digital-heirloom 验证权限');
    console.log('   3. 如果无法访问，请检�?RBAC 权限是否正确初始�?);
