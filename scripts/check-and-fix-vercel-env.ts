/**
 * 检查并修复 Vercel 环境变量脚本
 * 确保所有必需的环境变量都正确设置，特别是 signin 功能所需的变量
 * 
 * 使用方法：
...
if (!VERCEL_TOKEN) {
  ...
  console.error('   请设置环境变量: VERCEL_TOKEN=your-token pnpm tsx scripts/check-and-fix-vercel-env.ts');
  process.exit(1);
}
...
const REQUIRED_ENV_VARS = {
  ...
  DATABASE_URL: {
    ...
    description: '数据库连接 URL（必须使用连接池 URL，端口 6543）',
    critical: true,
    checkFormat: true, // 需要检查格式
  },
  
  // 认证配置（Signin 必需）
  AUTH_SECRET: {
    value: '', // 需要生成
    description: '认证密钥（使用 openssl rand -base64 32 生成）',
    critical: true,
    generate: true,
  },
  AUTH_URL: {
...
async function main() {
  ...
  for (const wrongName of WRONG_VAR_NAMES) {
    if (currentEnvVars[wrongName]) {
      ...
      for (const envVar of currentEnvVars[wrongName]) {
        ...
        if (deleted) {
          console.log(`  ✅ 已删除: ${wrongName} (${envVar.target})`);
          deletedCount++;
        }
      }
    }
    ...
    if (envVars.length === 0) {
      ...
      if (productionVar) {
        ...
        if (config.checkFormat && key === 'DATABASE_URL') {
          ...
          // 如果值看起来是加密的（不以 postgres:// 开头），我们需要更新它
          if (!currentValue.startsWith('postgres://') || 
            ...
            console.log(`     当前值: ${currentValue.substring(0, 80)}...`);
  ...
  console.log(`✅ 已修复缺失/错误变量: ${fixedCount} 个`);
  console.log(`❌ 仍需手动检查: ${missing.length - fixedCount} 个`);
