 ...
 * 确保使用正确的连接池 URL（端�?6543，包�?pgbouncer=true�?
 */
...
async function main() {
  ...
  if (databaseUrlVars.length === 0) {
    ...
    for (const envVar of databaseUrlVars) {
      console.log(`  🗑�? 删除旧的 DATABASE_URL (${envVar.target?.join(', ') || 'unknown'})...`);
  ...
  if (!validateDatabaseUrl(CORRECT_DATABASE_URL)) {
    ...
    console.error('   应包�? pooler.supabase.com:6543, pgbouncer=true, postgres.vkafrwwskupsyibrvcvd');
    process.exit(1);
  }
  ...
  if (success) {
    ...
    console.log('💡 重要提示�?);
    console.log('   1. DATABASE_URL 现在使用连接�?URL（端�?6543�?);
