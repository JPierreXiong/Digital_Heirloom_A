...
async function testConnection() {
  ...
  try {
    ...
    console.log(`   ‚ú?Database URL: ${databaseUrl.substring(0, 50)}...`);
    console.log('   ‚ú?Environment variables configured\n');
    ...
    try {
      ...
      console.log(`   ‚ú?Query result: ${JSON.stringify(result[0])}`);
      console.log('   ‚ú?Database connection works\n');
    } catch (error: any) {
      console.error('   ‚ù?Database connection failed:');
    ...
    try {
      ...
      const tablesResult = await sql`
        ...
        LIMIT 5
      `;
      console.log(`   ‚ú?Found ${tablesResult.length} tables in public schema`);
      if (tablesResult.length > 0) {
        console.log('   ‚ú?Sample tables:');
        tablesResult.forEach((row: any) => {
          console.log(`     - ${row.table_name}`);
        });
      }
      console.log('   ‚ú?Table queries work\n');
    } catch (error: any) {
    ...
    console.error('\n‚ù?Database connection test failed!');
    console.error(`Error: ${error.message}`);
    if (error.cause) {
      console.error(`Cause: ${error.cause.message || error.cause}`);
    }
