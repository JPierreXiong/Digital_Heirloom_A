...
async function testConnection(name: string, url: string) {
  ...
  try {
    ...
    try {
      ...
      console.log(`   ✅ config 表查询成功: ${configResult[0].count} 条记录`);
    } catch (e: any) {
      console.log(`   ⚠️  config 表查询失败: ${e.message}`);
    }
    
    // 测试查询 user 表
    try {
      const userResult = await sql`SELECT COUNT(*) as count FROM "user"`;
      console.log(`   ✅ user 表查询成功: ${userResult[0].count} 条记录`);
    } catch (e: any) {
      console.log(`   ⚠️  user 表查询失败: ${e.message}`);
    }
...
async function main() {
  ...
  // 测试连接池 URL
  results.pooler = await testConnection('连接池 URL (推荐)', connectionStrings.pooler);
  ...
  // 如果都失败，测试使用 postgres 作为用户名
  if (!results.pooler && !results.direct) {
    console.log('\n⚠️  标准连接失败，尝试使用 postgres 作为用户名...');
    results.postgresOnly = await testConnection('postgres 用户名', connectionStrings.postgresOnly);
  }
  ...
  if (results.pooler) {
    console.log('\n✅ 推荐使用连接池 URL（已测试成功）');
    console.log(`\n在 Vercel 中设置 DATABASE_URL 为:`);
    console.log(connectionStrings.pooler);
  } else if (results.direct) {
    console.log('\n⚠️  连接池 URL 失败，但直接连接成功');
    console.log('注意：直接连接不适合 Vercel 生产环境，可能导致连接数超限');
    console.log(`\n临时解决方案：在 Vercel 中使用直接连接 URL:`);
    console.log(connectionStrings.direct);
  } else if (results.postgresOnly) {
    console.log('\n⚠️  标准连接失败，但使用 postgres 用户名成功');
    console.log('这可能意味着 Supabase 项目配置有问题');
  } else {
    console.log('\n❌ 所有连接测试都失败了！');
    console.log('\n可能的原因：');
    console.log('1. Supabase 项目密码已更改');
    console.log('2. Supabase 项目被暂停或删除');
    console.log('3. 项目引用 ID 不正确');
    console.log('4. 网络连接问题');
    console.log('\n建议：');
    console.log('1. 登录 Supabase Dashboard 检查项目状态');
    console.log('2. 在 Settings → Database 中重新生成连接字符串');
