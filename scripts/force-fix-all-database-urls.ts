...
async function main() {
  console.log('🔧 强制修复所�?DATABASE_URL 变量...\n');
  ...
  for (const envVar of databaseUrlVars) {
    ...
    console.log(`    �? ${isEncrypted ? '[已加密]' : value.substring(0, 60) + '...'}`);
    console.log(`    类型: ${envVar.type || 'plain'}`);
    console.log('');
  }

  // 删除所�?DATABASE_URL 变量（无论环境）
  console.log('🗑�? 删除所有现有的 DATABASE_URL 变量...\n');
  const databaseUrlOnlyVars = envVars.filter((e: any) => e.key === 'DATABASE_URL');
  
  for (const envVar of databaseUrlOnlyVars) {
    const targets = Array.isArray(envVar.target) ? envVar.target.join(', ') : envVar.target || 'unknown';
    console.log(`  🗑�? 删除 DATABASE_URL (${targets})...`);
  ...
  console.log(`📋 找到 ${finalDatabaseUrlVars.length} �?DATABASE_URL 变量:\n`);
  
  for (const envVar of finalDatabaseUrlVars) {
    const targets = Array.isArray(envVar.target) ? envVar.target.join(', ') : envVar.target || 'unknown';
    console.log(`  �?DATABASE_URL`);
    console.log(`     环境: ${targets}`);
    console.log(`     �? [已加�?- 用户名格�? postgres.vkafrwwskupsyibrvcvd]`);
    console.log('');
  }
  ...
  console.log('�?修复完成！\n');
  console.log('💡 重要提示�?);
  console.log('   1. �?所有环境的 DATABASE_URL 已更�?);
  console.log('   2. �?用户名格式：postgres.vkafrwwskupsyibrvcvd');
  console.log('   3. �?使用连接�?URL（端�?6543�?);
  console.log('   4. �?包含 pgbouncer=true');
  console.log('   5. ⚠️  必须重新部署项目才能使更改生�?);
