 ...
 * 
 * 功能�? * 1. 查找或创建测试用户和保险�? * 2. 模拟活跃检测（设置 lastSeenAt �?40 天前�? * 3. 运行 Cron Job 触发预警邮件
 * 4. 验证状态变�?PENDING_VERIFICATION
 * 5. 模拟触发继承（设�?updatedAt �?8 天前�? * 6. 再次运行 Cron Job 触发 Dead Man's Switch
 * 7. 验证状态变�?TRIGGERED
 */
  ...
  try {
    ...
    if (!testUserId) {
      ...
      if (users.length === 0) {
        ...
        console.log('   请先创建一个用户和保险箱，或设�?TEST_USER_ID 环境变量\n');
        process.exit(1);
      }
    ...
    const daysAgo = heartbeatFrequency + 5; // 超过心跳频率5天，进入预警�?
    ...
    console.log(`   �?已将 lastSeenAt 设置�? ${daysAgoDate.toISOString()} (${daysAgo} 天前)`);
    console.log(`   �?心跳频率: ${heartbeatFrequency} 天`);
    console.log(`   �?状态已重置�? ACTIVE`);
    console.log(`   �?Dead Man's Switch: 启用\n`);

    console.log('   📧 运行 Cron Job 来触发预警邮�?..\n');
    ...
    try {
      ...
      if (cronResponse.ok) {
        ...
        console.log(`      警告邮件发�? ${cronResult.warningsSent || 0}`);
        console.log(`      提醒邮件发�? ${cronResult.remindersSent || 0}`);
    ...
    // 等待一下让数据库更�?    await new Promise((resolve) => setTimeout(resolve, 2000));
    ...
    // 计算宽限期结束日期（lastSeenAt + heartbeatFrequency + gracePeriod�?    const gracePeriod = vault.gracePeriod || 7;
    ...
    // 设置为宽限期结束日期之后1天（确保已超过宽限期�?    // �?updatedAt 应该设置为过去的时间，表示进入宽限期的时�?    const now = new Date();
    const daysInGracePeriod = 6; // 6天前进入宽限�?    const enteredGracePeriodDate = new Date(now);
    ...
    await db()
      ...
      .set({
        ...
        updatedAt: enteredGracePeriodDate, // 设置为进入宽限期的时�?        warningEmailSentAt: enteredGracePeriodDate,
        warningEmailCount: 3, // 已发�?�?        // 保持 lastSeenAt 不变，因�?findVaultsNeedingAssetRelease 使用它来计算宽限�?      })
      .where(eq(digitalVaults.id, vault.id));

    console.log(`   �?已将状态设置为: PENDING_VERIFICATION`);
    console.log(`   �?已将 updatedAt 设置�? ${triggerDate.toISOString()}`);
    console.log(`   �?宽限期结束日�? ${gracePeriodEndDate.toISOString()}`);
    ...
    console.log('\n   📧 运行 Cron Job 来触�?Dead Man\'s Switch...\n');
    ...
    try {
      ...
      if (cronResponse2.ok) {
        ...
        console.log('   �?Cron Job 执行成功');
        console.log(`      警告邮件发�? ${cronResult2.warningsSent || 0}`);
        console.log(`      提醒邮件发�? ${cronResult2.remindersSent || 0}`);
