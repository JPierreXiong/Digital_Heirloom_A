...
async function testEndpoint(
  ...
  try {
    ...
    // 注意：由于这是服务器端测试，我们需要模拟认证或使用服务端直接调�?    // 这里我们只测试端点是否存在和返回正确的响应格�?
    ...
    // 这里我们只测试端点是否可访问（可能会返回 401，但至少端点存在�?    const response = await fetch(`${BASE_URL}${endpoint}`, options);
    const data = await response.json();

    if (response.status === 200 || response.status === 401) {
      // 401 表示端点存在但需要认证，这是正常�?      return {
        name,
        endpoint,
        method,
...
async function testAdminDashboard() {
  ...
  // Phase 1: 核心看板和列表页�?  console.log('📊 Phase 1: 核心看板和列表页�?);
  results.push(await testEndpoint('统计信息', '/api/admin/digital-heirloom/stats'));
  results.push(await testEndpoint('高风险金库列�?, '/api/admin/digital-heirloom/vaults?urgent=true'));

  // Phase 2: 补偿功能和审计日�?  console.log('\n📝 Phase 2: 补偿功能和审计日�?);
  results.push(await testEndpoint('补偿审计日志', '/api/admin/digital-heirloom/compensations'));

  // Phase 3: 成本监控和安全监�?  console.log('\n💰 Phase 3: 成本监控和安全监�?);
  ...
  results.push(await testEndpoint('自定义报�?, '/api/admin/digital-heirloom/reports?type=overview'));
  ...
  console.log('\n' + '='.repeat(60));
  console.log('💡 说明:');
  console.log('   - "跳过"状态表示端点存在但需要认证，这是正常�?);
  console.log('   - 要完整测试功能，请使用浏览器登录后访问页�?);
  console.log('   - 所有端点都应该返回正确的响应格式（即使�?401�?);
  console.log('='.repeat(60) + '\n');
