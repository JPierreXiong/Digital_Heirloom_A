 ...
 * 
 * 验证内容�? * 1. API Key �?Signing Secret 是否正确配置
...
async function verifyCreemConfig() {
  ...
  try {
    ...
    if (configs.creem_api_key === EXPECTED_CONFIG.apiKey) {
      ...
      console.log(`     �? ${configs.creem_api_key.substring(0, 20)}...`);
    } else if (configs.creem_api_key) {
      console.log('  ⚠️  API Key 已配置，但与预期值不�?);
      console.log(`     当前�? ${configs.creem_api_key.substring(0, 20)}...`);
      console.log(`     预期�? ${EXPECTED_CONFIG.apiKey.substring(0, 20)}...`);
    } else {
      console.log('  �?API Key 未配�?);
      allPassed = false;
    }
    console.log('');
    ...
    if (configs.creem_signing_secret === EXPECTED_CONFIG.signingSecret) {
      ...
      console.log(`     �? ${configs.creem_signing_secret.substring(0, 20)}...`);
    } else if (configs.creem_signing_secret) {
      console.log('  ⚠️  Signing Secret 已配置，但与预期值不�?);
      console.log(`     当前�? ${configs.creem_signing_secret.substring(0, 20)}...`);
      console.log(`     预期�? ${EXPECTED_CONFIG.signingSecret.substring(0, 20)}...`);
    } else {
      console.log('  �?Signing Secret 未配�?);
      allPassed = false;
    }
    console.log('');
    ...
    if (configs.creem_product_ids) {
      try {
        ...
        if (baseProductId === EXPECTED_CONFIG.products.base.creemProductId) {
          ...
          console.log('  ⚠️  Base 产品映射存在，但值不�?);
          console.log(`     当前�? ${baseProductId}`);
          console.log(`     预期�? ${EXPECTED_CONFIG.products.base.creemProductId}`);
          allPassed = false;
        } else {
        ...
        if (proProductId === EXPECTED_CONFIG.products.pro.creemProductId) {
          ...
          console.log('  ⚠️  Pro 产品映射存在，但值不�?);
          console.log(`     当前�? ${proProductId}`);
          console.log(`     预期�? ${EXPECTED_CONFIG.products.pro.creemProductId}`);
          allPassed = false;
        } else {
      ...
      console.log('  �?Product IDs 映射未配�?);
      console.log('     需要配�?creem_product_ids，格�?');
      console.log('     {');
    ...
    if (configs.creem_enabled === 'true') {
      console.log('  �?Creem 已启�?);
    } else {
      console.log('  �?Creem 未启�?);
      console.log('     需要设�?creem_enabled = true');
      allPassed = false;
    }

    if (configs.creem_environment === 'production') {
      console.log('  �?Environment 设置�?production');
    } else {
      console.log(`  ⚠️  Environment: ${configs.creem_environment || '未设�?} (建议使用 production)`);
    }

    if (configs.default_payment_provider === 'creem') {
      console.log('  �?默认支付提供商设置为 creem');
    } else {
      console.log(`  ⚠️  默认支付提供�? ${configs.default_payment_provider || '未设�?} (建议设置�?creem)`);
    }
    console.log('');
