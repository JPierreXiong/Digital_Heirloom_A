/**
 * 完整流程自动化测试脚�? * 用途：模拟从用户注册到触发物理快递的全部流程
...
// 确保 DATABASE_URL 已设�?if (!process.env.DATABASE_URL && !process.env.POSTGRES_URL) {
  console.error('�?错误: DATABASE_URL �?POSTGRES_URL 未设�?);
  console.error(`   请检�?.env.local 文件: ${envPath}`);
  process.exit(1);
}
...
async function testCompleteShippingFlow() {
  ...
  try {
    ...
    logStep(3, '创建数字保险�?, '模拟用户完成 Step 1-4 设置');
    ...
    console.log(`�?数字保险箱创建成�?(ID: ${vaultId})`);

    // ============================================
    // Step 4: 创建受益人（包含完整地址信息�?    // ============================================
    logStep(4, '创建受益�?, `受益人邮�? ${beneficiaryEmail}`, {
    ...
    console.log(`   状�? ${ShippingStatus.PENDING_REVIEW}`);
    console.log(`   运费状�? ${ShippingFeeStatus.NOT_REQUIRED}`);

    // ============================================
    // 人工干预提醒 1: 管理员核算运�?    // ============================================
    console.log('\n' + '⚠️'.repeat(30));
    console.log('🛑 需要人工干�?- Step 1: 管理员核算运�?);
    ...
    console.log('   5. 点击 "发送支付链�?');
    console.log('\n💡 提示:');
    console.log(`   - 受益人邮�? ${beneficiaryEmail}`);
    console.log('   - 检查受益人邮箱是否收到支付请求邮件');
    console.log('\n');

    await askQuestion('�?完成以上操作后，�?Enter 继续...\n');

    // ============================================
    // Step 7: 验证支付链接已发�?    // ============================================
    logStep(7, '验证支付链接已发�?, '检查物流请求状�?);
    ...
    if (!updatedLog) {
      throw new Error('物流请求不存�?);
    }

    if (updatedLog.shippingFeeStatus === ShippingFeeStatus.PENDING_PAYMENT) {
      console.log('�?支付链接已发送，状态已更新�?pending_payment');
      console.log(`   支付链接: ${updatedLog.creemPaymentLink || '待创�?}`);
    } else {
      console.log(`⚠️  状�? ${updatedLog.shippingFeeStatus}`);
    ...
    // ============================================
    console.log('\n' + '⚠️'.repeat(30));
    console.log('🛑 需要人工干�?- Step 2: 模拟支付完成');
    console.log('⚠️'.repeat(30));
    console.log('\n📋 操作步骤（二选一�?');
    console.log('\n方式 A: 使用 SQL 脚本（推荐）');
    console.log('   1. �?Supabase SQL Editor 中执�?');
    console.log('      scripts/simulate-shipping-payment.sql');
    console.log(`   2. 或手动更新物流请�?ID: ${shippingLogId}`);
    console.log('\n方式 B: 通过 Creem Webhook（真实支付）');
    console.log('   1. 受益人点击支付链接完成支�?);
    console.log('   2. Creem Webhook 自动更新状�?);
    console.log('\n');

    await askQuestion('�?完成支付后，�?Enter 继续...\n');

    // ============================================
    // Step 8: 验证支付完成
    // ============================================
    logStep(8, '验证支付完成', '检查物流请求状�?);
    ...
    if (paidLog?.shippingFeeStatus === ShippingFeeStatus.PAID) {
      console.log('�?支付已完成，状态已更新�?paid');
      console.log(`   支付时间: ${paidLog.paidAt || '待确�?}`);
    } else {
      console.log(`⚠️  当前状�? ${paidLog?.shippingFeeStatus || '未知'}`);
    ...
    // 人工干预提醒 3: 管理员确认发�?    // ============================================
    console.log('\n' + '⚠️'.repeat(30));
    console.log('🛑 需要人工干�?- Step 3: 管理员确认发�?);
    console.log('⚠️'.repeat(30));
    console.log('\n📋 操作步骤:');
    console.log('   1. 访问: http://localhost:3000/admin/shipping-requests');
    console.log(`   2. 找到物流请求 ID: ${shippingLogId}`);
    ...
    if (finalLog?.status === ShippingStatus.SHIPPED) {
      console.log('�?发货已完成！');
      console.log(`   物流单号: ${finalLog.trackingNumber || '待确�?}`);
      console.log(`   承运�? ${finalLog.carrier || '待确�?}`);
      console.log(`   发货时间: ${finalLog.shippedAt || '待确�?}`);
    } else {
      console.log(`⚠️  当前状�? ${finalLog?.status || '未知'}`);
