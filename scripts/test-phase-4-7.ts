/**
 * Phase 4-7 快速测试脚�? * 用于快速验证核心功能是否正常工�? * 
 * 使用方法�? * npx tsx scripts/test-phase-4-7.ts
 */
...
async function testPhase4() {
  ...
  // 注意：这些测试需要实际的 Vault ID，请替换为测试数�?  const testVaultId = process.env.TEST_VAULT_ID || 'test-vault-id';
  
  if (testVaultId === 'test-vault-id') {
    console.log('   ⚠️  跳过 Phase 4 测试：TEST_VAULT_ID 未设�?);
    console.log('   💡 请设置环境变量：export TEST_VAULT_ID="your-vault-id"\n');
    return;
  }
  
  try {
    // 测试 4.1: 存储限制检�?    console.log('1. 测试存储限制检�?..');
    const storageCheck = await checkStorageLimit(testVaultId, 11 * 1024); // 11KB，超�?Free 限制
    console.log('   �?存储限制检�?', storageCheck.allowed ? '通过' : '限制生效');
    if (!storageCheck.allowed) {
      console.log('   📝 限制原因:', storageCheck.reason);
    }
    
    // 测试 4.2: 受益人数量限制检�?    console.log('2. 测试受益人数量限制检�?..');
    const beneficiaryCheck = await checkBeneficiaryLimit(testVaultId);
    console.log('   �?受益人限制检�?', beneficiaryCheck.allowed ? '通过' : '限制生效');
    ...
    // 测试 4.3: 心跳频率限制检�?    console.log('3. 测试心跳频率限制检�?..');
    const frequencyCheck = await checkHeartbeatFrequency(testVaultId, 29); // 29 天，低于 Base 最小限�?    console.log('   �?心跳频率限制检�?', frequencyCheck.allowed ? '通过' : '限制生效');
    if (!frequencyCheck.allowed) {
      console.log('   📝 限制原因:', frequencyCheck.reason);
      console.log('   📊 允许范围:', frequencyCheck.min, '-', frequencyCheck.max, '�?);
    }
...
async function testPhase6() {
  ...
  if (testToken === 'test-token') {
    console.log('   ⚠️  跳过 Phase 6 测试：TEST_RELEASE_TOKEN 未设�?);
  ...
  try {
    ...
    if (authResult.valid && authResult.context) {
      ...
      console.log('   📝 受益�?', authResult.context.beneficiary.name);
      ...
      console.log('   ⚠️ Token 验证失败（这是正常的，如果使用测�?Token�?);
...
async function main() {
  ...
  // 检查环境变�?  const testVaultId = process.env.TEST_VAULT_ID;
  const testToken = process.env.TEST_RELEASE_TOKEN;
  
  if (!testVaultId || testVaultId === 'test-vault-id') {
    console.log('⚠️  警告：TEST_VAULT_ID 未设置或使用默认�?);
    console.log('   💡 请设置环境变量：export TEST_VAULT_ID="your-vault-id"\n');
  }
  
  if (!testToken || testToken === 'test-token') {
    console.log('⚠️  警告：TEST_RELEASE_TOKEN 未设置或使用默认�?);
    console.log('   💡 请设置环境变量：export TEST_RELEASE_TOKEN="your-token"\n');
  }
