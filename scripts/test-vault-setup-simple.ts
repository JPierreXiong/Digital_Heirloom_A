/**
 * 简化版测试脚本：直接测试保险箱初始�?API
 * 
 * 功能�? * 1. 模拟完整的保险箱设置数据
 * 2. 调用初始�?API
 * 3. 验证结果
 * 
 * 注意：需要先登录系统获取有效�?session cookie
 */
...
const TEST_CONFIG = {
  ...
  sessionCookie: process.env.TEST_SESSION_COOKIE || '', // 从浏览器开发者工具获�?  masterPassword: 'TestPassword123!@#',
...
// 简化的加密函数（Node.js 环境�?async function encryptDataNode(plainText: string, password: string) {
  const salt = crypto.randomBytes(16);
  const iv = crypto.randomBytes(12); // GCM 需�?12 字节 IV
...
// 生成助记词（简化版�?function generateMnemonic(): string {
  const words = [
...
class VaultSetupTester {
  ...
  async run() {
    ...
    try {
      ...
      // Step 3: 验证保险箱创�?      await this.step3_VerifyVault();
  ...
  private async step1_PrepareData() {
    ...
    try {
      ...
      // 加密主密码备�?      const { encryptedData: backupToken, salt: backupSalt, iv: backupIv } = 
  ...
  private async step2_InitializeVault() {
    console.log('📝 Step 2: 调用初始�?API...');
    
    try {
      if (!this.testData) {
        throw new Error('测试数据未准�?);
      }

      // 准备请求�?      const headers: Record<string, string> = {
        'Content-Type': 'application/json',
      };

      // 如果�?session cookie，添加到请求�?      if (TEST_CONFIG.sessionCookie) {
      ...
      // 验证返回的数�?      if (!result.data || !result.data.vault) {
  ...
  private async step3_VerifyVault() {
    console.log('🔍 Step 3: 验证保险箱创�?..');
    
    try {
      if (!this.vaultId) {
        throw new Error('Vault ID 未找�?);
      }
      ...
      // 调用 API 获取保险箱信�?      const response = await fetch(`${TEST_CONFIG.baseUrl}/api/digital-heirloom/vault/get`, {
        method: 'GET',
        headers,
      });
      ...
      console.log('验证 API 响应状�?', response.status);
      console.log('验证 API 响应数据:', JSON.stringify(result, null, 2));

      if (result.code !== 200 || !result.data?.vault) {
        throw new Error(result.message || '无法获取保险箱信�?);
      }

      const vault = result.data.vault;

      // 验证保险箱状�?      if (vault.status !== 'active') {
        throw new Error(`保险箱状态不正确: 期望 'active'，实�?'${vault.status}'`);
      }

      // 验证受益人数�?      if (!result.data.beneficiaries || result.data.beneficiaries.length !== TEST_CONFIG.beneficiaries.length) {
        throw new Error(
          `受益人数量不正确: 期望 ${TEST_CONFIG.beneficiaries.length}，实�?${result.data.beneficiaries?.length || 0}`
        );
      }
      ...
      if (vault.heartbeatFrequency !== TEST_CONFIG.settings.heartbeatFrequency) {
        throw new Error(
          `心跳频率不正�? 期望 ${TEST_CONFIG.settings.heartbeatFrequency}，实�?${vault.heartbeatFrequency}`
        );
      }

      if (vault.gracePeriod !== TEST_CONFIG.settings.gracePeriod) {
        throw new Error(
          `宽限期不正确: 期望 ${TEST_CONFIG.settings.gracePeriod}，实�?${vault.gracePeriod}`
        );
      }
  ...
  private printResults() {
    ...
    if (failCount === 0) {
      ...
      console.log('📋 测试总结:');
      console.log(`   - 保险�?ID: ${this.vaultId}`);
      console.log(`   - 状�? active`);
      console.log(`   - 受益人数�? ${TEST_CONFIG.beneficiaries.length}`);
      console.log(`   - 心跳频率: ${TEST_CONFIG.settings.heartbeatFrequency} 天`);
      console.log(`   - 宽限�? ${TEST_CONFIG.settings.gracePeriod} 天`);
      console.log('');
    } else {
      ...
      console.log('      b. 打开开发者工�?(F12)');
      console.log('      c. 进入 Application/Storage -> Cookies');
      console.log('      d. 找到 better-auth.session_token cookie');
      console.log('      e. 复制完整�?cookie �?);
      console.log('   4. 运行测试时设置环境变�?');
      console.log('      $env:TEST_SESSION_COOKIE="better-auth.session_token=你的token�?; npx tsx scripts/test-vault-setup-simple.ts');
      console.log('   5. 或者直接在代码中设�?TEST_CONFIG.sessionCookie');
      console.log('');
      process.exit(1);
    }
  }
...
if (require.main === module && !TEST_CONFIG.sessionCookie) {
  ...
  console.log('⚠️  测试需要认�?);
  console.log('========================================\n');
  console.log('请先登录系统，然后获�?session cookie：\n');
  console.log('1. 打开浏览器，访问 http://localhost:3000');
  console.log('2. 登录系统');
  console.log('3. 打开开发者工�?(F12)');
  console.log('4. 进入 Application/Storage -> Cookies');
  console.log('5. 找到 better-auth.session_token cookie');
  console.log('6. 复制完整�?cookie 值\n');
  console.log('然后运行：\n');
  console.log('  PowerShell:');
  console.log('    $env:TEST_SESSION_COOKIE="better-auth.session_token=你的token�?; npx tsx scripts/test-vault-setup-simple.ts\n');
  console.log('  Bash:');
  console.log('    TEST_SESSION_COOKIE="better-auth.session_token=你的token�? npx tsx scripts/test-vault-setup-simple.ts\n');
  console.log('========================================\n');
}
