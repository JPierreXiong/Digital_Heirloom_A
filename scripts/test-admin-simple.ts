...
async function testAdminSimple() {
  ...
  try {
    ...
    if (!adminUser) {
      console.error(`�?未找到用�? ${ADMIN_EMAIL}`);
      return;
    }
    ...
    try {
      ...
      console.log('�?digital_vaults 表查询正�?);
    } catch (error: any) {
      console.log(`�?digital_vaults 表查询失�? ${error.message}`);
    }

    // admin_audit_logs
    try {
      const logs = await db().select().from(adminAuditLogs).limit(1);
      console.log('�?admin_audit_logs 表查询正�?);
    } catch (error: any) {
      console.log(`�?admin_audit_logs 表查询失�? ${error.message}`);
    }

    // email_notifications
    try {
      const emails = await db().select().from(emailNotifications).limit(1);
      console.log('�?email_notifications 表查询正�?);
    } catch (error: any) {
      console.log(`�?email_notifications 表查询失�? ${error.message}`);
    }

    // beneficiaries
    try {
      const bens = await db().select().from(beneficiaries).limit(1);
      console.log('�?beneficiaries 表查询正�?);
    } catch (error: any) {
      console.log(`�?beneficiaries 表查询失�? ${error.message}`);
    }

    // system_alerts
    try {
      const alerts = await db().select().from(systemAlerts).limit(1);
      console.log('�?system_alerts 表查询正�?);
    } catch (error: any) {
      console.log(`�?system_alerts 表查询失�? ${error.message}`);
    }
