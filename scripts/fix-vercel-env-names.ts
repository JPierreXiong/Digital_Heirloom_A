 ...
 * 自动修复 Vercel 环境变量命名
 * 
 * 使用方法�? * VERCEL_TOKEN=your-token pnpm tsx scripts/fix-vercel-env-names.ts
 */
...
// 需要重命名的变量映�?const RENAME_MAP: Record<string, string> = {
...
async function main() {
  ...
  // 找出需要重命名的变�?  const varsToRename: Array<{
    oldVar: VercelEnvVar;
    newKey: string;
  }> = [];
  ...
  for (const { oldVar, newKey } of varsToRename) {
    // 跳过已存在的变量
    if (existingKeys.has(newKey)) {
      console.log(`⏭️  跳过 ${oldVar.key} �?${newKey} (目标变量已存�?`);
      continue;
    }
    
    console.log(`🔄 重命�?${oldVar.key} �?${newKey}...`);
    
    // 1. 创建新变�?    const target = oldVar.target || ['production', 'preview', 'development'];
  ...
  if (successCount > 0) {
    ...
    console.log('  2. 确认所有环境（Production, Preview, Development）都已勾�?);
    console.log('  3. 重新部署项目（Redeploy�?);
    console.log('  4. 如果仍有旧变量残留，请手动删�?);
  }
