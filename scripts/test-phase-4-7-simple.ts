...
console.log('📋 环境变量检�?');
console.log(`   TEST_VAULT_ID: ${testVaultId ? '�?已设�? : '�?未设�?}`);
console.log(`   TEST_RELEASE_TOKEN: ${testToken ? '�?已设�? : '�?未设�?}\n`);

if (!testVaultId || testVaultId === 'test-vault-id') {
  console.log('⚠️  警告：TEST_VAULT_ID 未设�?);
...
if (!testToken || testToken === 'test-token') {
  console.log('⚠️  警告：TEST_RELEASE_TOKEN 未设�?);
...
console.log('   2. �?测试脚本已创�?);
console.log('   3. �?测试文档已创�?);
console.log('   4. �?等待设置测试数据\n');

console.log('📋 下一�?');
console.log('   1. 获取测试 Vault ID �?Release Token（参�?TESTING_QUICK_START.md�?);
console.log('   2. 设置环境变量');
console.log('   3. 运行完整测试脚本: npx tsx scripts/test-phase-4-7.ts');
console.log('   4. 或按�?TESTING_GUIDE_PHASE_4_7.md 进行详细测试\n');
