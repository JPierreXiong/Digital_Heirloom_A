...
function validateDatabaseUrl(url: string): { valid: boolean; issues: string[] } {
  ...
  if (!url.includes('postgres.vkafrwwskupsyibrvcvd')) {
    if (url.includes('postgres://postgres@')) {
      issues.push('用户名格式错误：使用�?"postgres" 而不�?"postgres.vkafrwwskupsyibrvcvd"');
    } else {
...
async function main() {
  console.log('🔍 验证并修�?DATABASE_URL...\n');

  // 验证正确的连接字符串格式
  const validation = validateDatabaseUrl(CORRECT_DATABASE_URL);
  if (!validation.valid) {
    console.error('�?正确�?DATABASE_URL 格式验证失败:');
    validation.issues.forEach(issue => console.error(`   - ${issue}`));
    process.exit(1);
  }
  console.log('�?正确�?DATABASE_URL 格式验证通过\n');
  ...
  if (databaseUrlVars.length === 0) {
    console.log('�?未找�?DATABASE_URL 变量，将创建新的...\n');
  } else {
    console.log(`📋 找到 ${databaseUrlVars.length} �?DATABASE_URL 变量\n`);
    ...
    if (needsUpdate || databaseUrlVars.length > 0) {
      ...
      for (const envVar of databaseUrlVars) {
        console.log(`  🗑�? 删除 DATABASE_URL (${envVar.target?.join(', ') || 'unknown'})...`);
  ...
  if (success) {
    ...
    console.log('   2. �?使用连接�?URL（端�?6543�?);
    console.log('   3. �?包含 pgbouncer=true 参数');
    console.log('   4. ⚠️  必须重新部署项目才能使更改生�?);
