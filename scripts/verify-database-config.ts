/**
 * 验证数据库配置脚�? * 检�?DATABASE_URL 是否正确配置，并测试数据库连�? */
...
async function verifyDatabaseConfig() {
  ...
  // 1. 检查环境变�?  const databaseUrl = process.env.DATABASE_URL;
  const databaseProvider = process.env.DATABASE_PROVIDER || 'postgresql';

  console.log('📋 环境变量检�?');
  console.log(`   DATABASE_URL: ${databaseUrl ? '�?已设�? : '�?未设�?}`);
  ...
  try {
    ...
    console.log(`   数据�? ${url.pathname.replace('/', '')}`);
    console.log(`   SSL 模式: ${url.searchParams.get('sslmode') || '未指�?}\n`);
  } catch (error) {
    console.warn('⚠️  无法解析 DATABASE_URL，但将继续测试连�?..\n');
  }

  // 3. 验证 envConfigs 是否读取到环境变�?  const { envConfigs } = await import('@/config');
  console.log('📋 envConfigs 检�?');
  console.log(`   database_url: ${envConfigs.database_url ? '�?已设�? : '�?未设�?}`);
  if (!envConfigs.database_url) {
    console.error('\n�?envConfigs.database_url 为空�?);
    console.log('💡 这可能是模块加载顺序问题�?);
    console.log('💡 请确�?.env.local 文件存在且包�?DATABASE_URL\n');
    process.exit(1);
  }

  // 4. 测试数据库连�?  console.log('\n🔌 测试数据库连�?..');
  try {
    const database = db();
    
    // 尝试执行一个简单查�?    const result = await database.execute('SELECT 1 as test');
    
    console.log('   �?数据库连接成功！');
    console.log(`   📊 测试查询结果: ${JSON.stringify(result)}\n`);
    
    // 5. 测试查询 digital_vaults 表（如果存在�?    try {
      const { digitalVaults } = await import('@/config/db/schema');
      const vaults = await database.select().from(digitalVaults).limit(1);
      console.log('   �?可以访问 digital_vaults �?);
      console.log(`   📊 表中�?${vaults.length > 0 ? '数据' : '无数�?}\n`);
    } catch (error: any) {
    ...
    console.log('💡 故障排除建议:');
    console.log('   1. 检�?DATABASE_URL 是否正确');
    console.log('   2. 检查网络连�?);
    console.log('   3. 检查数据库是否允许来自当前 IP 的连�?);
