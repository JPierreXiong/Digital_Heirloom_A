...
async function diagnosePaymentConfig() {
  ...
  try {
    ...
    if (configs.paypal_enabled === 'true') {
      ...
      console.log('  �?PayPal: 已启�?);
    } else {
      console.log('  �?PayPal: 未启�?);
    }
    ...
    if (defaultProvider) {
      console.log(`  �?已设�? ${defaultProvider}`);
    } else {
    ...
    if (configs.creem_enabled === 'true') {
      ...
      if (configs.creem_product_ids) {
        try {
          ...
          console.log(`  �?Product IDs Mapping: 已配�?${productCount} 个产品映射`);
          console.log('     映射的产�?');
        ...
        console.log('  �?Product IDs Mapping: 未配�?);
        console.log('     ⚠️  这是导致支付失败的主要原因！');
        console.log('     💡 解决方案:');
        console.log('        1. 访问 Creem Dashboard: https://www.creem.io/dashboard/products');
        console.log('        2. 为每�?pricing item 创建对应的产�?);
        console.log('        3. �?admin settings 中配�?creem_product_ids，格�?');
        console.log('           {');
    ...
    if (configs.stripe_enabled === 'true') {
      ...
      if (configs.stripe_secret_key && configs.stripe_publishable_key) {
        console.log('  �?API Keys: 已配�?);
      } else {
        console.log('  �?API Keys: 未完整配�?);
    ...
    if (configs.paypal_enabled === 'true') {
      ...
      if (configs.paypal_client_id && configs.paypal_client_secret) {
        console.log('  �?Credentials: 已配�?);
      } else {
        console.log('  �?Credentials: 未完整配�?);
