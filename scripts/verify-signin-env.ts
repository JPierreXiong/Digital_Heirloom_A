...
async function main() {
  console.log('🔍 验证 Signin 功能所需的环境变量...\n');
  ...
  for (const key of SIGNIN_REQUIRED_VARS) {
    ...
    if (!anyVar) {
      ...
      if (key === 'DATABASE_URL') {
        ...
        // 如果值看起来是加密的（不以 postgres:// 开头），我们假设它是正确的
        ...
        if (isValid) {
          if (isEncrypted) {
            console.log(`  ✅ ${key}: 已设置（已加密，使用连接池 URL）[${envList}]`);
          } else {
            console.log(`  ✅ ${key}: 已设置（连接池 URL 格式正确）[${envList}]`);
          }
        } else {
          console.log(`  ⚠️  ${key}: 已设置但格式可能不正确 [${envList}]`);
          console.log(`     当前值: ${value.substring(0, 80)}...`);
          console.log(`     应该包含: pooler.supabase.com:6543 和 pgbouncer=true`);
          allOk = false;
        }
