...
async function main() {
  console.log('🔍 诊断 Vercel 生产环境数据库连接问�?..\n');
  ...
  // 1. 检查环境变�?
  console.log('📋 1. 检�?Production 环境变量...\n');
  ...
  if (dbUrlVar) {
    ...
    console.log(`    状�? ${isEncrypted ? '已加密（正常�? : '未加密（可能有问题）'}`);
    ...
    if (!isEncrypted) {
      ...
      console.log(`    💡 如果仍有问题，请确认使用的是连接�?URL（端�?6543）`);
    }
  } else {
  ...
  if (deployments.length > 0) {
    ...
    console.log(`    状�? ${latest.readyState || 'N/A'}`);
  ...
  console.log('   - 确认项目状态为 Active（不�?Paused�?);
  ...
  console.log('   - 复制连接字符串（应包�?pooler.supabase.com:6543�?);
  console.log('   - 确认密码�? tQbCJXRaLlABMRE6');
  ...
  console.log('\n4. 查看 Vercel 函数日志�?);
  console.log('   - Vercel Dashboard �?Deployments �?最新部�?�?Functions');
  console.log('   - 查找 [DB Error] �?[Config] 开头的日志');
  console.log('   - 查看详细的错误信�?);
  
  console.log('\n5. 重新部署�?);
  console.log('   - �?Vercel Dashboard 中触发重新部�?);
  console.log('   - 或运�? vercel --prod');
